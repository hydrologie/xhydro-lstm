...
xhydro-lstm could always use more documentation, whether as part of the official xhydro-lstm docs, in docstrings, or even on the web in blog posts, articles, and such.
...
Ready to contribute? Here's how to set up ``xhydro-lstm`` for local development.

#. First, clone the ``xhydro-lstm`` repo locally.

    * If you are not a ``xhydro-lstm`` collaborator, first fork the ``xhydro-lstm`` repo on GitHub, then clone your fork locally.

        .. code-block:: console

            git clone git@github.com:your_name_here/xhydro-lstm.git

    * If you are a ``xhydro-lstm`` collaborator, clone the ``xhydro-lstm`` repo directly.
...
#. Before merging, your Pull Request will need to be based on the `main` branch of the ``xhydro-lstm`` repository. If your branch is not up-to-date with the `main` branch, you can perform similar steps as above to update your branch:
...
You will have contributed to ``xhydro_lstm``!
...
.. code-block:: console
    ...
    python -m flake8 src/xhydro_lstm tests
    validate-docstrings src/xhydro_lstm/**.py
...
Please note that this project is released with a `Contributor Code of Conduct <https://github.com/hydrologie/xhydro_lstm/blob/main/CODE_OF_CONDUCT.md>`_.
